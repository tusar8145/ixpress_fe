...
//file.type = 'file'
  ...
  const onDownload = () => {
    ...
    link.download = `sample_booking.xlsx`;
    link.href = "/sample_booking.xlsx";
    link.click();
  };
  ...
  const handlesendsms = () =>{    
    ...
    const firsr_page_json={
     ...
     "i_product_type_id_2":firstpage.i_product_type_id || 529,
     "i_priority_id_3":firstpage.i_priority_id || 4,
     ...
     "i_payment_type_8":firstpage.i_payment_type || 5,
     "creator":user.id,
 ...
 console.log(final_submtted_data);
              //common validation
              if (firsr_page_json.services_id_1 > 0 && firsr_page_json.i_product_type_id_2 > 0 && firsr_page_json.i_priority_id_3 > 0 && firsr_page_json.i_payment_type_8 > 0) {
                setshow_error(0)
                let clearance = 1;
...
/*
                              ...
                              }).catch(function(error) {  if (error.response) {  setunique_upload_id(null) }   });
                              setswitch_data_submitted(1)
                              setswitch_data_submitted(0)
                          } else {
			...
			console.log(error)
			setProgress(null)
			 setunique_upload_id(null)
		   setalert_def_txt('Fail! Required Fields missing')
                            setalert_def_class('warning')
                            handleClick()
							setloading(0)
		}
...
const [force_print8,setforce_print8]  = useState(0);
	  ...
	  const [json_upload_update, setjson_upload_update] = useState(null);
	  
	  
      const [show_error, setshow_error] = useState(0);
      const [unique_upload_id, setunique_upload_id] = useState(null);
	  
	  const [no_of_booking, setno_of_booking] = useState(null);
...
var cod_=obj.collection_amount*obj.cod_cost_percent/100
          ...
          var x_data = {
              ...
              i_priority: obj.i_priority.i_priority || 4,
  ...
  const UpreadUploadFiles = (e) => { e.preventDefault(); 
    ...
    try {
          if (e.target.files) { 
              ...
              setswitch_xlsx_active(1) 
                  ...
                  const SenderRecipientMax2 = SenderRecipientRef.current.getSenderRecipient(); 
                ...
                for (var i = 0; i < json.length; i++){
                 // console.log('checking');
                  ...
                  var customer_id,id,product_info,product_type_id,packaging_type_id,payment_type_id,delivery_cost_amount,collection_amount,cod_cost_percent,return_cost_amount;
                  try { 
                       ...
                       /* if(!(name.length>0  &&   reference.length>0)){
						...
						if(!id>0){
							   error_msg='Fail! XLSX Line: '+line+'. id not found ';
                                console.log(error_msg)
                                error=1;
                                break;  
						}
                        ...
                        if(product_type_id>0){
						  ...
						  console.log(i_product_typeList,'i_product_typeList')
                          var filter2 =[]
                          filter2 = filterItemsequal(i_product_typeList,'value',product_type_id);
                          if (!filter2.length>0){
                        ...
                        if(packaging_type_id>0){
                          //is present on product type upon under the service
                          var filter2 =[]
                          filter2 = filterItemsequal(i_packaging_typeList,'value',packaging_type_id);
                          if (!filter2.length>0){
                        ...
                        const dta={
						  ...
						  id:id,
						  ...(name ? {recipient_name_21: name, } : {}),
						  ...(phone ? {recipient_phone_20: phone, } : {}),
						  ...(address1 ? {recipient_address_24: address1,} : {}),
						  ...(reference ? {sender_ref_no_4: reference,} : {}),
						  
                          ...(firstpage.i_priority_id ? { i_priority_id_3: firstpage.i_priority_id } : {}),
						  ...(firstpage.services_id ? { i_priority_id_3: firstpage.i_priority_id } : {}),
                          ...
                          ...(product_type_id ? { i_product_type_id_2: product_type_id } : {
							  ...
							  ...(firstpage.i_product_type_id ? {i_product_type_id_2: firstpage.i_product_type_id, } : {}),
							  
						  }),
						  
						  ...(SenderRecipientMax2.i_sms_template_id_30 ? {i_sms_template_id_30: SenderRecipientMax2.i_sms_template_id_30, } : {}),
			  
						  ...(SenderRecipientMax2.sender_client_id_7 ? {sender_client_id_7: SenderRecipientMax2.sender_client_id_7, } : {}),
						  ...(SenderRecipientMax2.sender_client_branch_id_8 ? {sender_client_branch_id_8: SenderRecipientMax2.sender_client_branch_id_8, } : {}),
                ...
                if(error==1){
                  ...
                  handleClick()
					...
					//update to server
						const res =    axios.post(base_url + "update_shipment_multiple", new_excel_json).then((response) => {
						console.log('response', response)
			  
							setalert_def_txt('Data Updated Successfully');  setalert_def_class('success');  handleClick();
                   ...
                   setjson_upload_update(new_excel_json);
          ...
          if (no_of_booking) { 
                ...
                for (var i = 0; i < no_of_booking; i++){
                  ...
                  try { 
                        ...
                        delivery_cost_amount=null
                        collection_amount=null
                        cod_cost_percent=null
                        return_cost_amount=null
           

                        const dta={
                          recipient_name_21: "",
                          recipient_phone_20: "",
                          recipient_address_24: "",
                          sender_ref_no_4: ran,
                          ...(customer_id ? { customer_no: customer_id } : {}),
                          ...(product_info ? { product_info_4	: product_info } : {}),
                          ...(product_type_id ? { i_product_type_id_2: product_type_id } : {}),
                          ...(packaging_type_id ? { i_packaging_type_id_16: 3 } : {}),
                          ...(payment_type_id ? { i_payment_type_8: payment_type_id } : {}),
                          ...(delivery_cost_amount ? { delivery_cost_amount: delivery_cost_amount } : {}),
                          ...(collection_amount ? { collection_amount: collection_amount } : {}),
                          ...(cod_cost_percent ? { cod_cost_percent: cod_cost_percent } : {}),
...
<Grid container spacing={6}> 
  <Grid item md={3} lg={3}   sm={12} xs={12}>
          ...
          color="success"   variant="contained"  component="label">
              ...
              &nbsp;<input   name="upload"  id="upload"  onChange={UpreadUploadFiles} accept="XLSX/*" multiple type="file" />
...
<Grid item md={3} lg={3}   sm={12} xs={12}>
  ...
  <Grid item md={9}  lg={9}   sm={12} xs={12}> 
	 ...
	 <Grid item md={3}  lg={3}   sm={12} xs={12}>	
      ...
      value={i_product_type_id} id="i_product_type_id" SelectPodType renderInput={(params) => ( <TextField {...params} label="Select Product Type: (Optional)" variant="standard" /> )} />
 ...
 <Grid item md={3}  lg={3}   sm={12} xs={12}>
    <Autocomplete {...i_priorityProps}  disableClearable        onChange={(event, value)=>{   setfirstpage(previousState => {  return { ...previousState, i_priority_id: value.value }  }); }}   id="i_priority_id" SelectPodType renderInput={(params) => ( <TextField {...params} label="Select Product Priority: (Optional)" variant="standard" /> )} />    
    { firstpage.i_priority_id == null && show_error==1 && <div> <p style={{color: "red", marginTop:"-10px"}}>This field is required</p>  </div> }
              ...
              id="i_payment_type_id" SelectPodType renderInput={(params) => ( <TextField {...params} label="Select Payment Type (Optional)" variant="standard" /> )} />  
...
<SenderRecipient ref={SenderRecipientRef}   
    ...
    i_packaging_typeList={i_packaging_typeList}
...
<Grid container spacing={6}>
    ...
    <Grid item md={3}   lg={3}   sm={12} xs={12} justify="flex-end">
              <TextField type="number" inputProps={ {  min: 1, max: 5000  } } 
			  onFocus={event => {
        event.target.select();
      }}
			    min={1}
 ...
 onChange={(e) => {
	 setno_of_booking(e.target.value)

	 console.log(e.target.value)
       
      }}
			  
			  label="POD Counts:*  "   validators={["required"]}/>
